...
@implementation PlannerItem

@synthesize text, checked, priority, dueDate, shouldRemind, itemId, toDelete;
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    if ((self = [super init])) {
        ...
        self.priority = [aDecoder decodeIntForKey:@"Priority"];
...
{
    if (self = [super init]) {
        self.itemId = [DataModel nextPlannerItemId];
    }
    return self;
}
...
- (NSComparisonResult)compare:(PlannerItem *)otherPlannerItem
{
    
    return [self.dueDate compare:otherPlannerItem.dueDate];
}
